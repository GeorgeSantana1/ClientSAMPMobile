...
CSettings::CSettings()
{
	...
	// Client
	short length = 0;
	sprintf(buff, "Javanes_%d%d", rand() % 1000, rand() % 1000);
	length = reader.Get("client", "name", buff).copy(m_Settings.szNickName, MAX_PLAYER_NAME);
	m_Settings.szNickName[length] = '\0';
	length = reader.Get("client", "CHost", "127.0.0.1").copy(m_Settings.szHost, MAX_SETTINGS_STRING);
	...
	m_Settings.iPort = reader.GetInteger("client", "CPort", 7777);
