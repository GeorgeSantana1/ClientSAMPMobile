...
void SvrStats(RPCParameters *rpcParams)
{
	...
	bsData.Read((char *)&pRakStats, sizeof(RakNetStatisticsStruct));
}
...
void DamageVehicle(RPCParameters *rpcParams)
{
	...
	if(pVehicle) 
	{
		pVehicle->UpdateDamageStatus(VehicleID, dwPanels, dwDoors, byteLights);
	}
}
...
void Update3DTextLabel(RPCParameters *rpcParams)
{
	...
	if(pLabelsPool)
	{
		pLabelsPool->Update3DLabel(LabelID, color, szBuff);
	}
}
