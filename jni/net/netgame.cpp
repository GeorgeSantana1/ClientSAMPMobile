...
CNetGame::~CNetGame()
{
	...
	if(pDialogWindow->m_bIsActive && pButton->m_bIsActive && pScoreBoard->m_bIsActive)
	{
...
void CNetGame::Packet_PlayerSync(Packet* pkt)
{
	...
	bsPlayerSync.Read(byteTempWeapon);
	ofSync.byteCurrentWeapon ^= (byteTempWeapon ^ ofSync.byteCurrentWeapon) & 0x3F;
...
{
	CRemotePlayer * pPlayer;
	RakNet::BitStream bsSpectatorSync((unsigned char *)pkt->data, pkt->length, false);
