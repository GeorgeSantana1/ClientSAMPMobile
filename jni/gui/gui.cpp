...
void CGUI::Render()
{
	...
	ImGui::GetForegroundDrawList()->AddText(ImVec2(ScaleX(0), ScaleY(1060)), 0xFFFFFFFF, "INDONESIA");
...
{
	ImGui::GetForegroundDrawList()->AddText(ImVec2(ScaleX(10), ScaleY(0)), 0xFFFFFFFF, "0.95");
}

void CGUI::RenderRakNetStatistics()
{
	RakNetStatisticsStruct pRakStats;

	if((GetTickCount() - m_dwLastUpdateTick) > 1000) {
		m_dwLastUpdateTick = GetTickCount();
		
		m_dwBPSDownload = ((uintptr_t)(pRakStats.bitsReceived / 8)) - m_dwLastTotalBytesRecv;
		m_dwLastTotalBytesRecv = (uintptr_t)(pRakStats.bitsReceived / 8);

		m_dwBPSUpload = ((uintptr_t)(pRakStats.totalBitsSent / 8)) - m_dwLastTotalBytesSent;
		m_dwLastTotalBytesSent = (uintptr_t)(pRakStats.totalBitsSent / 8);
	}
	...
	//StatisticsToString(&pRakStats, szSvrStatBuf, 1);
	//strcat(szSvrDispBuf, szSvrStatBuf);
	
	ImGui::GetForegroundDrawList()->AddText(ImVec2(ScaleX(10), ScaleY(1027)), 0xFFFFFFFF, szSvrDispBuf);
}
...
void CGUI::RenderText(ImVec2& posCur, ImU32 col, bool bOutline, const char* text_begin, const char* text_end)
{
	...
	{
		posCur.x -= iOffset;
		ImGui::GetForegroundDrawList()->AddText(posCur, ImColor(IM_COL32_BLACK), text_begin, text_end);
		posCur.x += iOffset;
		// right 
		posCur.x += iOffset;
		ImGui::GetForegroundDrawList()->AddText(posCur, ImColor(IM_COL32_BLACK), text_begin, text_end);
	...
	ImGui::GetForegroundDrawList()->AddText(posCur, col, text_begin, text_end);
}