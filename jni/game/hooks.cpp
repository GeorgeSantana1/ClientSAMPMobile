...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	if(!strncmp(r1, "DATA/WEAPON.DAT", 18))
	{
...
uint16_t* CText_Get_hook(uintptr_t thiz, const char* text)
{
	if(text[0] == 'S' && text[1] == 'A' && text[2] == 'M' && text[3] == 'P')
	{
		...
		if(!strcmp(code, "MP")) AsciiToGxtChar("MultiPlayer", gxt_string);
...
void CLoadingScreen_DisplayPCScreen_hook()
{
	...
	if(RwCameraBeginUpdate(camera))
	{
		...
		(( void (*)())(g_libGTASA+0x3D7854+1))(); // CLoadingScreen::RenderSplash() 
		
		if(percent >= 0.01f)
		{
			(( void (*)())(g_libGTASA+0x3D7B2C+1))(); // CLoadingScreen::RenderLoadingBar()
		}
...
extern "C" bool NotifyEnterVehicle(VEHICLE_TYPE *_pVehicle)
{
	...
	if(pLocalPlayer->GetPlayerPed() && pLocalPlayer->GetPlayerPed()->GetCurrentWeapon() == WEAPON_MODEL_PARACHUTE)
