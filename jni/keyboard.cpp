...
void CKeyBoard::Render()
{
	...
	// input string
	if(pDialogWindow->m_pPassEntering != 1)
	{
		ImGui::GetForegroundDrawList()->AddText(pGUI->GetFont(), m_fFontSize, ImVec2(m_Pos.x + m_Size.x * 0.02, m_Pos.y + m_Pos.y * 0.05), 0xFFFFFFFF, m_utf8Input);
	}else{
		if(strlen(m_utf8Input) == 0) m_pPasswordSymbol = "";
		if(strlen(m_utf8Input) == 1) m_pPasswordSymbol = "*";
		if(strlen(m_utf8Input) == 2) m_pPasswordSymbol = "**";
		if(strlen(m_utf8Input) == 3) m_pPasswordSymbol = "***";
		if(strlen(m_utf8Input) == 4) m_pPasswordSymbol = "****";
		if(strlen(m_utf8Input) == 5) m_pPasswordSymbol = "*****";
		if(strlen(m_utf8Input) == 6) m_pPasswordSymbol = "******";
		if(strlen(m_utf8Input) == 7) m_pPasswordSymbol = "*******";
		if(strlen(m_utf8Input) == 8) m_pPasswordSymbol = "********";
		if(strlen(m_utf8Input) == 9) m_pPasswordSymbol = "*********";
		if(strlen(m_utf8Input) == 10) m_pPasswordSymbol = "**********";
		if(strlen(m_utf8Input) == 11) m_pPasswordSymbol = "***********";
		if(strlen(m_utf8Input) == 12) m_pPasswordSymbol = "************";
		if(strlen(m_utf8Input) == 13) m_pPasswordSymbol = "*************";
		if(strlen(m_utf8Input) == 14) m_pPasswordSymbol = "**************";
		if(strlen(m_utf8Input) == 15) m_pPasswordSymbol = "***************";
		if(strlen(m_utf8Input) == 16) m_pPasswordSymbol = "****************";
		if(strlen(m_utf8Input) == 17) m_pPasswordSymbol = "*****************";
		if(strlen(m_utf8Input) == 18) m_pPasswordSymbol = "******************";
		if(strlen(m_utf8Input) == 19) m_pPasswordSymbol = "*******************";
		if(strlen(m_utf8Input) == 20) m_pPasswordSymbol = "********************";
		if(strlen(m_utf8Input) == 21) m_pPasswordSymbol = "*********************";
		if(strlen(m_utf8Input) == 22) m_pPasswordSymbol = "**********************";
		if(strlen(m_utf8Input) == 23) m_pPasswordSymbol = "***********************";
		if(strlen(m_utf8Input) == 24) m_pPasswordSymbol = "************************";
		if(strlen(m_utf8Input) == 25) m_pPasswordSymbol = "*************************";
		if(strlen(m_utf8Input) == 26) m_pPasswordSymbol = "**************************";
		if(strlen(m_utf8Input) == 27) m_pPasswordSymbol = "***************************";
		if(strlen(m_utf8Input) == 28) m_pPasswordSymbol = "****************************";
		if(strlen(m_utf8Input) == 29) m_pPasswordSymbol = "*****************************";
		if(strlen(m_utf8Input) == 30) m_pPasswordSymbol = "******************************";
		if(strlen(m_utf8Input) == 31) m_pPasswordSymbol = "*******************************";
		if(strlen(m_utf8Input) >= 32) m_pPasswordSymbol = "********************************";

		sprintf(m_pPassInputBuffer, "%s", m_pPasswordSymbol);
		ImGui::GetForegroundDrawList()->AddText(pGUI->GetFont(), m_fFontSize, ImVec2(m_Pos.x + m_Size.x * 0.02, m_Pos.y + m_Pos.y * 0.05), 0xFFFFFFFF, m_pPassInputBuffer);
	}
	
	// dividing line
	ImGui::GetForegroundDrawList()->AddLine(ImVec2(m_Pos.x, m_Pos.y + m_fKeySizeY), ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), 0xAA3333AA);
	...
	for(int i=0; i<4; i++)
	{
		...
		{
			if(key.id == m_iPushedKey && key.type != KEY_SPACE && pDialogWindow->m_pPassEntering != 1)
				ImGui::GetForegroundDrawList()->AddRectFilled(key.pos, ImVec2(key.pos.x + key.width, key.pos.y + fKeySizeY), 0xAA3333AA);
			...
			switch(key.type)
			{
				...
				case KEY_SHIFT:
					ImGui::GetForegroundDrawList()->AddTriangleFilled(
						...
						m_iCase == LOWER_CASE ? 0xFF8A8886 : 0xAA3333AA);
				...
				ImGui::GetForegroundDrawList()->AddRectFilled(
					...
					key.id == m_iPushedKey ? 0xAA3333AA : 0xFF8A8886);
				break;
...
void CKeyBoard::Send()
{
	if(m_pHandler)
	{
		if(m_sInput == "/q" or m_sInput == "/quit")
		{
...
void CKeyBoard::InitENG()
{
	...
	// Fix Crash On Click MultiPlayer Button
	key.pos = curPos;
	key.symPos = ImVec2(1000,1000);
	key.width = 0;
	key.code[LOWER_CASE] = ' ';
	key.code[UPPER_CASE] = ' ';
